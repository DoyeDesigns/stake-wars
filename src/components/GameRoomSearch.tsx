...
const GameRoomSearch = ({setIsOpen} : GameRoomSearchProps) => {
  ...
  return (
    <div className="w-full space-y-4 mb-5 pt-3">
      ...
      {error && (
        <div className="bg-[#919090]/80 text-white text-center !mb-4">
            <span>{error}</span>
        </div>
      )}
        ...
        >
          <div className="p-5 h-fit">
            ...
            <div className="grid lg:grid-cols-2 gap-2 text-[12px] lg:text-[14px]">
              ...
              <div className='flex justify-between gap-1 lg:justify-start'>
                <strong>Staked Amount:</strong> <span className='text-[#BFE528] text-right lg:text-left'>{gameRoom.stakeDetails?.stakeAmount} g{gameRoom.stakeDetails?.symbol}</span>
              </div>
